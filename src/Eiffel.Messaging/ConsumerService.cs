...
namespace Eiffel.Messaging
{
    public class ConsumerService : IHostedService
    {
        ...
        {
            var tasks = GetConsumerTasks(cancellationToken);
        ...
        private IEnumerable<Task> GetConsumerTasks(CancellationToken cancellationToken)
        {
            foreach(var route in _messageRouteRegistry.Routes)
            {
                ...
                yield return (Task)method.Invoke(_messageBus, new object[] { route.Item2, cancellationToken });
            }
        }
    }
}
